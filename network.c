...
network_t* make_network() {
  ...
  //printf("The integer is %d,%d,%d", net->layers[1]->width, net->layers[1]->height, net->layers[1]->depth);
  ...
  //printf("The integer is %d,%d,%d", net->layers[4]->width, net->layers[4]->height, net->layers[4]->depth);
  ...
  //printf("The integer is %d,%d,%d", net->layers[7]->width, net->layers[7]->height, net->layers[7]->depth);
  ...
  //printf("The integer is %d,%d,%d", net->layers[9]->width, net->layers[9]->height, net->layers[9]->depth);
...
void net_classify(network_t* net, volume_t** input, double** likelihoods, int n) {
  ...
  double* b_weights = b[11][0]->weights;
  int num_threads   = omp_get_num_threads();
  int chunksize     = n / num_threads;
  int bound         = num_threads * chunksize;
  int thread_num = omp_get_thread_num();
  for (int i = thread_num * chunksize; i < (thread_num + 1) * chunksize; i++) {
    copy_volume(b[0][0], input[i]);
    net_forward(net, b, 0, 0);
    for (int j = 0; j < NUM_CLASSES; j++) {
      likelihoods[i][j] = b_weights[j];
    }
  }
  if (n % num_threads > 0) {
